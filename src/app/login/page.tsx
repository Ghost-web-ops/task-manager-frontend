...
import { useAuth } from '@/context/AuthContext'; // <-- استيراد الـ hook الخاص بنا
...
export default function LoginPage() {
  ...
  const { login } = useAuth(); // <-- استخدام الـ context للحصول على دالة login
  ...
  const handleSubmit = async (e: FormEvent) => {
   ...
   ;
    try {
      ...
      if (res.ok) {
        login(data.token); // <-- هنا نستخدم دالة login من الـ context لحفظ التوكن
        router.push('/'); // توجيه المستخدم إلى الصفحة الرئيسية بعد النجاح
      } else {
   ...
   return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900">
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <h1 className="text-2xl font-bold text-center text-gray-900 dark:text-gray-100">Login to your Account</h1>
